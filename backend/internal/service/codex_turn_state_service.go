...
func (s *CodexTurnStateService) ProbeAccountModel(ctx context.Context, accountID int64, model string, force bool) (*CodexProbeResult, error) {
	...
	var saveErr error
	var inspErr error
	var insp *CodexTurnStateInspection

	if result.StatusCode == 200 && result.TurnState != "" {
		insp, inspErr = InspectCodexTurnState(result.TurnState)
		if inspErr != nil || insp == nil || !insp.Valid || insp.IsExpired {
			if inspErr != nil {
				result.ErrorMessage = fmt.Sprintf("invalid turn-state: %v", inspErr)
			} else if insp != nil && insp.IsExpired {
				result.ErrorMessage = "probe returned already expired turn-state"
			} else {
				result.ErrorMessage = "probe returned invalid turn-state"
			}
		} else {
			saveErr = s.saveTurnState(ctx, accountID, model, result, insp)
			if saveErr != nil {
				result.ErrorMessage = fmt.Sprintf("failed to persist turn-state: %v", saveErr)
	...
	success := result.StatusCode == 200 && result.TurnState != "" && inspErr == nil && insp != nil && insp.Valid && !insp.IsExpired && saveErr == nil
	...
	if !success && probeErr == nil && result.ErrorMessage != "" {
		probeErr = errors.New(result.ErrorMessage)
	}
...
func (s *CodexTurnStateService) GetProxyConfig(ctx context.Context) (*CodexTurnStateProxyConfig, error) {
	...
	if err := json.Unmarshal([]byte(val), cfg); err == nil && (len(cfg.StaticProxies) > 0 || len(cfg.DynamicProxies) > 0 || cfg.CookieRenewalSeconds > 0) {
		...
		validStatic := make([]string, 0, len(cfg.StaticProxies))
		for _, p := range cfg.StaticProxies {
			if norm := normalizeProxyURL(p); norm != "" {
				validStatic = append(validStatic, norm)
			}
		}
		cfg.StaticProxies = validStatic

		validDynamic := make([]string, 0, len(cfg.DynamicProxies))
		for _, p := range cfg.DynamicProxies {
			if norm := normalizeProxyURL(p); norm != "" {
				validDynamic = append(validDynamic, norm)
			}
		}
		cfg.DynamicProxies = validDynamic
