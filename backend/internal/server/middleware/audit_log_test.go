...
func TestCodexPanelImportAuditOmitsCredentialBody(t *testing.T) {
	gin.SetMode(gin.TestMode)
	repository := &auditCaptureRepository{}
	auditService := service.NewAuditLogService(repository, nil)
	auditService.Start()
	...
	router.Use(gin.HandlerFunc(NewAuditLogMiddleware(auditService)))
	router.POST("/api/v1/admin/codex-turn-state/*path", func(c *gin.Context) { c.JSON(http.StatusOK, gin.H{"ok": true}) })
	request := httptest.NewRequest(http.MethodPost, "/api/v1/admin/codex-turn-state/api/proxy-sources", bytes.NewBufferString(`{"type":"static","content":"example.invalid:8080:sample:audit-canary-password"}`))
	request.Header.Set("Content-Type", "application/json")
	recorder := httptest.NewRecorder()
	router.ServeHTTP(recorder, request)
	require.Equal(t, http.StatusOK, recorder.Code)
